...
All 24 gated metric cells pass. The largest observed upper bound is 2.190516% for baseline-versus-disabled F16 CPU TTFT against its 3.485397% budget. The tightest decode budget is 0.988906% on CUDA; its four observed upper bounds range from 0.043793% to 0.093985%. Two complete corrected captures are preserved because each had a different five-token F16 CPU prompt cell exceed its fixed noise gate. The final report selects the first passing complete ABBA capture independently for each issue-declared comparison and binds every selection to the unchanged raw attempt by SHA-256. These are control-derived noise gates for the tiny fixtures, not production regression tolerances.
