...
### Phase 8 cumulative Level E/F miss-execution subset

Issue #26 validates `PROMOTE_AND_GPU`, explicit `CPU_FALLBACK`, and explicit deterministic `AUTO` on the production miss-execution path. `PROMOTE_AND_GPU` remains the stable default. AUTO consumes a versioned caller snapshot, fails closed to GPU for absent or invalid operands, and selects GPU on ties; the runtime does not learn or explore online. Canonical top-k accumulation, fused gate/up/down dependencies, mixed hot-GPU/cold-CPU execution, explicit bounded background promotion, cancellation, unload, and closeout are covered by focused native tests and the 25-case production-path probe.

The standing parity capture covers original and generated 218-part split K3 F16/MXFP4 inputs with ten cold processes and two warm captures per case, plus a larger public MoE F16 bootstrap case. All cases preserve their numerical and policy gates. Descriptor-only PP/TG graph discovery records two graphs with zero scheduler reserve calls and zero backend allocation before bounded hot/cold/ring initialization and final `COLD_CACHE` reservation. Ten controlled mixed CPU/GPU repetitions record positive CPU work, GPU work, and overlap; overlap is 562 us minimum, 722 us p50, and 802 us p95/p99.

The controlled benchmark contains 300 cells: five policy/regime identities across decode and prefill, three hot ratios, two reuse classes, and background promotion off/on. It establishes explicitly constructed CPU-favorable, GPU-favorable, and tie decisions and records prompt/decode throughput, TTFT, token p50/p95/p99, H2D bytes, avoided current-output bytes, selected backend, selection reason, prediction digest, and regret. Observed real-model process-latency tails are retained separately: K3 cases are approximately 3.17–3.39 s p50, while the larger public MoE bootstrap case is 36.941 s p50. These process timings include setup and are not token-throughput comparisons.

The exact-layout synthetic store is derived from immutable `moonshotai/Kimi-K3` config revision `9f62e4e9fffbd0a83ddd60e1c209d828994b3569`. Its 92 layers, 896 experts per layer, 3,072 expert width, 3,584 latent width, three projections, and MXFP4 17-byte/32-element layout produce exactly 1,446,456,066,048 logical bytes. The local sparse payload allocates only 167,936 filesystem bytes and is verified through deterministic sampled spans. It validates size arithmetic, sparse positional reads, and controlled policy regimes; it is not full-model inference, a storage-throughput result for allocated media, or a quality result.

Focused CPU, CUDA, ASan+UBSan, and accepted ASLR-disabled TSan suites each pass 5/5. Phase 8 evidence tests pass 32/32, the Phase 7 authoritative manifest re-verifies at its accepted closeout head, and project/nested diff and exact-gitlink checks pass. Device-wide telemetry records 3,661 MiB peak used and 242 MiB minimum free. Default-ASLR TSan retains the accepted pre-test mapping limitation; the authoritative native run disables ASLR. Checkpoint C also carries an unchanged callback-free fixture teardown race as a non-material test-only note. Neither note weakens the corrected production/model paths.

### Phase 9 cumulative Level E/F cache-policy subset

Issue #30 validates model-owned, bounded policy state separated from hot/cold slot ownership, bytes, generations, references, publication, I/O, H2D, and miss execution. Exact global LRU compatibility plus deterministic LFRU, SLRU, frequency-gated optional hot admission, LFU-aging, WASTE-style sampled replay comparators, global/per-layer domains, heterogeneous byte planning, checked logical ordering, bounded transcripts, cancellation, surrender, and unload are covered by focused native tests. Runtime/native/Python replay agrees on the immutable Phase 2 corpus and required online transcripts; policy decisions do not consume wall time, backend completion order, physical-residency telemetry, or unordered iteration.

The final statistics matrix has 470 exact-identity artifacts: 8 screening runs, 22 warmups, and 440 measured runs spanning 11 comparisons. No non-LRU global pair passes every frozen default gate. Hot LFRU with cold LRU improves aggregate median token time by only about 0.0035%, its 95% interval includes zero, and worst policy CPU is about 2.13% of end-to-end token time. The two retained SLRU-containing pairs regress aggregate median token time by about 0.61–1.04%, include approximately 8% Qwen regressions, and also exceed the 1% policy-CPU gate. Exact global LRU/ALWAYS is therefore retained for both null defaults; explicit configurations remain available and per-layer policy remains explicit-only.

Physical-memory sweeps distinguish logical from resident hits and record RSS/PSS, process faults, swap availability/activity, physical residency, hit-service time, throughput, and token tails with explicit host headroom. The corrected full-K3 top-16 one-token working set is exactly 25,829,572,608 bytes; five safe exact-layout cells remain fully resident with zero major faults and zero swap growth, so no paging cliff was observed within the safe ceiling. This is memory/residency evidence, not full-model inference, quality, or storage-throughput evidence.

Scoped cold-budget recommendations are 44,040,192 bytes for the two-token tiny-K3 F16 `CPU_FALLBACK` boundary workload, 11,698,176 bytes for its MXFP4 counterpart, 1,678,245,888 bytes for the accepted one-token Qwen bootstrap only, and 25,829,572,608 bytes for the full-K3 exact-layout residency cell only. All preserve the recorded headroom and are evidence outputs, not hidden runtime auto-sizing. Repeated 20-token explicit-LRU versus null-default runs cover original/split tiny F16/MXFP4 hot and cold modes plus accepted Qwen cold mode; all nine groups preserve exact output identity and resolve to global LRU/ALWAYS with no semantic default switch.
