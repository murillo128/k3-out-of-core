...
def payload_chunks(
    ...
    length: int,
    *,
    output_chunk_bytes: int = 1 << 20,
) -> Iterator[bytes]:
    """Yield the exact v1 byte stream in bounded I/O-sized chunks.

    The evidence algorithm defines one SHA-256 digest for each 32-byte logical
    chunk.  Coalescing those digests before returning them changes neither the
    stream nor its identity, and avoids one positional-write syscall per
    digest for the full 25.8 GB corpus.
    """
    if length < 0:
        raise ValueError("negative payload length")
    if output_chunk_bytes < 32:
        raise ValueError("output chunk size must hold at least one digest")
    key = record_key(layer, expert, projection)
    remaining = length
    index = 0
    while remaining:
        target = min(remaining, output_chunk_bytes)
        output = bytearray()
        while len(output) < target:
            digest = hashlib.sha256(key + struct.pack("<Q", index)).digest()
            output.extend(digest[:target - len(output)])
            index += 1
        yield bytes(output)
        remaining -= target
...
class Journal:
    ...
    @staticmethod
    def truncate_to_complete_chains(path: Path) -> list[dict[str, object]]:
        """Discard a valid but incomplete final bundle chain after a restart."""
        records = Journal.read_valid(path, truncate=True)
        keep = len(records)
        while keep and keep % len(Journal.STATES):
            keep -= 1
        if keep:
            for index in range(0, keep, len(Journal.STATES)):
                states = [str(record["state"]) for record in records[index:index + len(Journal.STATES)]]
