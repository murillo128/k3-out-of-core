...
def main():
    ...
    root=Path.cwd().resolve(); exe=(a.cuda_build/"bin/phase5-cold-cache-probe").resolve(); cases=[]; hierarchy=[]
    for rep,original,token in (("f16",a.f16,"F16"),("mxfp4",a.mxfp4,"MXFP4")):
        ...
        for kind,model in (("original",original),("split",split)):
            hierarchy_record,hierarchy_diag=invoke(exe,root,model,"cold",3,67108864)
            hierarchy.append({"representation":rep,"kind":kind,"model":identity(root,model),"diagnostics":hierarchy_diag,"checks":{"command":hierarchy_record["exit_code"]==0,"cold_start_reads":hierarchy_diag["storage_read_requests"]>0,"cold_hits_without_reread":hierarchy_diag["cold_hits"]>0 and hierarchy_diag["storage_read_requests"]==hierarchy_diag["cold_misses"],"integrity_before_publication":hierarchy_diag["storage_integrity_checks"]==hierarchy_diag["storage_read_requests"] and hierarchy_diag["storage_integrity_mismatches"]==0},"output_digests":{"stdout":hierarchy_record["stdout_sha256"],"stderr":hierarchy_record["stderr_sha256"]}})
            captures=[]
            for capture in range(2):
                record,diag=invoke(exe,root,model,"cold",a.warm_epochs,7000000 if rep=="f16" else 2000000)
                checks={"command":record["exit_code"]==0,"exact_prompt":diag["prompt_ids"]==baseline["prompt_ids"],"exact_tokens":diag["tokens"]==baseline["tokens"],"exact_logits":diag["logits_hash"]==baseline["logits_hash"],"exact_routes":diag["route_hash"]==baseline["route_hash"] and diag["route_records"]==baseline["route_records"],"storage_used":diag["storage_read_requests"]>0 and diag["storage_read_bytes"]>0,"eviction_reread":diag["cold_evictions"]>0 and diag["storage_read_requests"]==diag["cold_misses"],"integrity":diag["storage_integrity_checks"]==diag["storage_read_requests"] and diag["storage_integrity_mismatches"]==0,"no_source_copy":diag["cold_source_bytes"]==0,"no_errors":diag["storage_short_reads"]==diag["storage_io_errors"]==diag["storage_cancelled_reads"]==0,"bounded":diag["cold_actual_bytes"]<=diag["cold_requested_bytes"] and diag["ring_actual_bytes"]<=diag["ring_requested_bytes"] and diag["ring_pinned_bytes"]<=diag["ring_actual_bytes"]}
            ...
            cases.append({"representation":rep,"kind":kind,"model":identity(root,model),"baseline_exit_code":base_run["exit_code"],"baseline":baseline,"baseline_output_digests":{"stdout":base_run["stdout_sha256"],"stderr":base_run["stderr_sha256"]},"captures":captures})
    status=all(all(all(x["checks"].values()) for x in c["captures"]) for c in cases) and all(all(x["checks"].values()) for x in hierarchy)
    write(a.output,{"schema_version":"phase6-demand-parity-v1","status":"pass" if status else "fail","warm_epochs":a.warm_epochs,"hierarchy":hierarchy,"cases":cases}); return 0 if status else 1
