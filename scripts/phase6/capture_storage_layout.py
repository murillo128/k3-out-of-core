...
def main():
    ...
    for name, model in (("f16",a.f16),("mxfp4",a.mxfp4)):
        ...
        for kind,path in (("original",model),("split",files[0])):
            ...
            admin_bound=65536+diag["storage_files"]*128+diag["storage_entries"]*64+diag["storage_spans"]*64
            checks={"command":record["exit_code"]==0,"no_routed_allocation":diag["deferred_allocated_bytes"]==0,"no_mmap_binding":diag["deferred_mmap_bound_bytes"]==0,"no_prefetch":diag["deferred_prefetch_bytes"]==0,"metadata_only":diag["routed_tensors"]==diag["routed_null"],"directory_complete":diag["storage_entries"]==56 and diag["storage_spans"]==168,"split_handles":diag["storage_files"]==(1 if kind=="original" else 218),"cross_split_bundle":kind=="original" or (diag["storage_files"]==218 and diag["storage_spans"]==168),"administration_bounded":0<diag["storage_admin_bytes"]<=admin_bound}
            diag["storage_admin_upper_bound"]=admin_bound
