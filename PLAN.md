...
Phase 9 issue #30 implements the bounded cache-policy framework, exact LRU compatibility, deterministic LFRU/SLRU/LFU-aging candidates, frequency-gated admission, hard global/per-layer domains, canonical replay, physical-residency evidence, and fixed-rule selection. Checkpoints A, B, and C returned `PASS`, safety `YES`, at comments `5148012128`, `5148752231`, and `5149625334`. No non-LRU pair passed every default gate, so exact global LRU/ALWAYS is retained for hot and cold tiers and per-layer policy remains explicit-only. Phase 9.5 closeout validation, the non-circular manifest, and final complete-PR review remain pending at this source-of-truth update.
